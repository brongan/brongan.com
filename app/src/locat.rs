...
pub fn locat() -> Result<std::sync::Arc<Locat>, ServerFnError> {
    use_context::<std::sync::Arc<Locat>>()
...
impl Db {
    async fn create(path: String) -> Result<Self, tokio_rusqlite::Error> {
        tracing::info!("Reading Sqlite3 db at: {}", path);
    ...
    async fn list(&self) -> Result<Vec<crate::analytics::Analytics>, tokio_rusqlite::Error> {
        self.connection
            .call(|connection| {
                let mut statement =
